...
class Trainer(AbstractTrainer):
    ...
    def __init__(self, config, model):
        ...
        self.logger = getLogger(name=config["model"])
    ...
    def _full_sort_batch_eval(self, batched_data):
        ...
        # print(scores, self.tot_item_num)
...
class DCRecTrainer(Trainer):
    def __init__(self, config, model):
        super(DCRecTrainer, self).__init__(config, model)
    ...
    @torch.no_grad()
    def evaluate(
        self, eval_data, load_best_model=True, model_file=None, show_progress=False, stage="test"
    ):
        if not eval_data:
            return
        ...
        for batch_idx, batched_data in enumerate(iter_data):
            num_sample += len(batched_data)
            interaction, scores, positive_u, positive_i = eval_func(batched_data, stage)
            if self.gpu_available and show_progress:
                iter_data.set_postfix_str(
                    set_color("GPU RAM: " + get_gpu_usage(self.device), "yellow")
                )
    ...
    def _full_sort_batch_eval(self, batched_data, stage):
        interaction, history_index, positive_u, positive_i = batched_data
        try:
            # Note: interaction without item ids
            scores = self.model.full_sort_predict(interaction.to(self.device), stage)
        except NotImplementedError:
            inter_len = len(interaction)
            new_inter = interaction.to(self.device).repeat_interleave(self.tot_item_num)
            batch_size = len(new_inter)
    ...
    def _neg_sample_batch_eval(self, batched_data, stage):
        interaction, row_idx, positive_u, positive_i = batched_data
        batch_size = interaction.length
        if batch_size <= self.test_batch_size:
            origin_scores = self.model.predict(interaction.to(self.device), stage)
        else:
            origin_scores = self._spilt_predict(interaction, batch_size)
...
class GCLRecTrainer(Trainer):
    def __init__(self, config, model):
        super(GCLRecTrainer, self).__init__(config, model)
    ...
    def _build_optimizer(self, **kwargs):
        ...
        opt = optim.Adam(
                [{'params': self.model.item_embedding.parameters(), 'lr': learning_rate},
                 {'params': self.model.user_embedding.parameters(), 'lr': learning_rate},
                 {'params': self.model.position_embedding.parameters(), 'lr': learning_rate},
                 {'params': self.model.mu.parameters(), 'lr': learning_rate},
                 {'params': self.model.logvar.parameters(), 'lr': learning_rate},
                 {'params': self.model.trm_encoder.parameters(), 'lr': learning_rate},
                 {'params': self.model.LayerNorm1.parameters(), 'lr': learning_rate},
                 {'params': self.model.LayerNorm2.parameters(), 'lr': learning_rate},
                 {'params': self.model.LayerNorm3.parameters(), 'lr': learning_rate},
                 {'params': self.model.LayerNorm4.parameters(), 'lr': learning_rate},
                 {'params': self.model.LayerNorm5.parameters(), 'lr': learning_rate},
                 {'params': self.model.attacker.parameters(), 'lr': learning_rate},
                 {'params': self.model.model.parameters(), 'lr': learning_rate},
                 {'params': self.model.gd.parameters(), 'lr': learning_rate},
                 {'params': self.model.softplus.parameters(), 'lr': learning_rate},
                 {'params': self.model.club.parameters(), 'lr': learning_rate},
                 ],
    ...
    def _train_epoch(self, train_data, epoch_idx, loss_func=None, show_progress=False):
        ...
        for batch_idx, interaction in enumerate(iter_data):
            ...
            self._check_nan(loss_gc)
            self._check_nan(loss_rec)
            # scaler.scale(loss_gc + sync_loss).backward(retain_graph=True)
            # self.model.trm_encoder.zero_grad()
            # scaler.scale(loss_rec + sync_loss).backward()
            scaler.scale(loss_rec + loss_gc + sync_loss).backward()
            if self.clip_grad_norm:
                clip_grad_norm_(self.model.parameters(), **self.clip_grad_norm)
            scaler.step(opt)
            scaler.update()
...
class ICLRecTrainer(Trainer):
    def __init__(self, config, model):
        super(ICLRecTrainer, self).__init__(config, model)
    ...
    def _train_epoch(self, train_data, epoch_idx, loss_func=None, show_progress=False):
        ...
        iter_cluster_data = (
            tqdm(
                ...
                total=len(train_data),
                ncols=100,
                desc=set_color(f"Train Cluster {epoch_idx:>5}", "pink"),
            )
            if show_progress
            else train_data
        )

        for batch_idx, interaction in enumerate(iter_cluster_data):
            interaction = interaction.to(self.device)
            item_seq = interaction[self.model.ITEM_SEQ]
            item_seq_len = interaction[self.model.ITEM_SEQ_LEN]
            sequence_output = self.model(item_seq, item_seq_len)
            kmeans_training_data.append(sequence_output.detach().cpu().numpy())
        
        kmeans_training_data = np.concatenate(kmeans_training_data, axis=0)

        print("Training Clusters:")
        for i, cluster in tqdm(enumerate(self.model.clusters), total=len(self.model.clusters)):
            cluster.train(kmeans_training_data)
            self.model.clusters[i] = cluster
        # clean memory
...
class CoseRecTrainer(Trainer):
    ...
    def _train_epoch(self, train_data, epoch_idx, loss_func=None, show_progress=False):
        self.model.similarity_model.update_embedding_matrix(self.model.item_embedding)
        self.model.train()
        loss_func = loss_func or self.model.calculate_loss
        total_loss = None
        iter_data = (
...
#     def __init__(self, config, model):
#         super(SASRecTrainer, self).__init__(config, model)
...
#                     set_color("GPU RAM: " + get_gpu_usage(self.device), "yellow")
#                 )
#         if epoch_idx % 1 == 0 and epoch_idx <= 21:
#             # print(self.saved_model_file[:-4] + '_epoch-{}.pth'.format(epoch_idx))
#             self._save_checkpoint(epoch_idx, True, saved_model_file=self.saved_model_file[:-4] + '_epoch-{}.pth'.format(epoch_idx))

#         return total_loss

# class CL4SRecTrainer(Trainer):
#     def __init__(self, config, model):
#         super(CL4SRecTrainer, self).__init__(config, model)
